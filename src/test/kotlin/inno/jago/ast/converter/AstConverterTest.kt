...
class AstConverterTest {
    ...
    fun `for with if and return`() {
        ...
        assertEquals(FunctionDeclarationNode::class.java, astRoot.topLevelDecls[0].javaClass,
            "Unexpected top level decl")
        ...
        assertEquals(ForClauseStatementNode::class.java, funDecl.functionBody.block[0].javaClass,
            "Unexpected number of stmts in function block")
        assertEquals(EmptyStatementNode::class.java, funDecl.functionBody.block[1].javaClass,
            "Unexpected number of stmts in function block")

        val forClause = funDecl.functionBody.block[0] as ForClauseStatementNode

        assertEquals(ShortVarDeclNode::class.java, forClause.initStatementNode?.javaClass,
            "Unexpected type of for init statement")
        ...
        assertEquals(RelationOperators.LESS, (condition.binaryOperator as RelationOperator).operator,
            "Unexpected relation operator")
        assertEquals(OperandNameNode::class.java, condition.leftExpression.javaClass,
            "Unexpected left relation operand")
        assertEquals(
            "i",
            ((condition.leftExpression as OperandNameNode).name as SimpleIdentifierOperandNode).identifier,
            "Unexpected left relation operand value"
        )
        assertEquals(LiteralOperandNode::class.java, condition.rightExpression.javaClass,
            "Unexpected right relation operand")
        assertEquals("3", ((condition.rightExpression as LiteralOperandNode).literalNode as BasicLiteralNode).value,
            "Unexpected right relation operand value")

        assertEquals(IncDecStatementNode::class.java, forClause.postStatementNode?.javaClass,
            "Unexpected type of for post statement")
        val post = (forClause.postStatementNode as IncDecStatementNode)
        assertEquals("i", ((post.expression as OperandNameNode).name as SimpleIdentifierOperandNode).identifier,
            "Unexpected left relation operand value")
        ...
        assertEquals(ReturnStatementNode::class.java, ifStmt.block.block[0].javaClass,
            "unexpected first statement inside of if")
        assertEquals(EmptyStatementNode::class.java, ifStmt.block.block[1].javaClass,
            "unexpected second statement inside of if")
        ...
        assertEquals(ApplicationExpressionNode::class.java, printExpr.expression.javaClass,
            "unexpected type of the second statement")
        ...
        assertEquals(
            "print",
            ((print.leftExpression as OperandNameNode).name as SimpleIdentifierOperandNode).identifier
        )
    ...
    fun `empty main fun must have only empty stmt`() {
        ...
        assertEquals(1, astRoot.topLevelDecls.size, "Unexpected number of toplevel decls")
        assertEquals(
            FunctionDeclarationNode::class.java, astRoot.topLevelDecls[0].javaClass,
            "Unexpected top level decl"
        )
        ...
        assertEquals(EmptyStatementNode::class.java, funDecl.functionBody.block[0].javaClass,
            "Unexpected number of stmts in function block")
    }
    ...
    fun `if node with decl must contain else branch`() {
        ...
        assertEquals(2, funDecl.functionBody.block.size, "Unexpected number of stmts in function block")
        assertEquals(IfStatementNode::class.java, funDecl.functionBody.block[0].javaClass,
            "Expected if stmt in fun block")
        assertEquals(EmptyStatementNode::class.java, funDecl.functionBody.block[1].javaClass,
            "Expected empty statement in fun block")
        ...
        assertEquals(ShortVarDeclNode::class.java, ifStmtNode.simpleStatement!!.javaClass,
            "Expected short var decl in if stmt declaration")
        assertEquals(
            "a",
            (ifStmtNode.simpleStatement!! as ShortVarDeclNode).identifierList[0],
            "Unexpected variable name"
        )

        assertEquals(LiteralOperandNode::class.java, ifStmtNode.expression.javaClass,
            "Expected literal operand in if condition expression")
        assertEquals(BoolLiteralNode::class.java,(ifStmtNode.expression as LiteralOperandNode).literalNode.javaClass,
            "Unexpected literal type in if condition expression")
        ...
        assertEquals(SimpleElseStatementNode::class.java, ifStmtNode.elseBranch!!.javaClass,
            "Expected simple else branch in if")
        assertEquals(2, (ifStmtNode.elseBranch!! as SimpleElseStatementNode).block.block.size,
            "Unexpected number of stmts in block of simple else")
    }
    ...
    fun `additional fun must have only return stmts`() {
        ...
        assertEquals(FunctionDeclarationNode::class.java, astRoot.topLevelDecls[0].javaClass,
            "Unexpected top level decl")
        assertEquals(FunctionDeclarationNode::class.java, astRoot.topLevelDecls[1].javaClass,
            "Unexpected top level decl")
        ...
        assertEquals(ReturnStatementNode::class.java, fooFunDecl.functionBody.block[0].javaClass,
            "Expected return stmt in foo block")
        ...
        assertEquals(ExpressionStatementNode::class.java, mainFunDecl.functionBody.block[0].javaClass,
            "Expected if stmt in fun block")
        assertEquals(
            ApplicationExpressionNode::class.java,
            (mainFunDecl.functionBody.block[0] as ExpressionStatementNode).expression.javaClass,
            "Expected application expression"
        )

        val applicationNode =
            (mainFunDecl.functionBody.block[0] as ExpressionStatementNode).expression as ApplicationExpressionNode
        assertEquals(OperandNameNode::class.java, applicationNode.leftExpression.javaClass,
            "Expected operand name node in application node")
        assertEquals(1, applicationNode.expressions.size, "Unexpected number of params in application node")
        assertEquals(LiteralOperandNode::class.java, applicationNode.expressions[0].javaClass,
            "Unexpected type of param in application node")
    }
    ...
    fun `top level var decl`() {
        ...
        assertEquals(LiteralOperandNode::class.java, varDecl.expression!!.javaClass,
            "Unexpected expression in var decl")
